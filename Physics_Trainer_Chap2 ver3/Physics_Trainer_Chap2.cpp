...
void Excersize_2_11(int solution, int answer)
{
	...
	printf("이 스포츠카는 %.0lf km/h의 속도로 달리다가 브레이크를 밟은 후 %.0lf m 이내에서 정지할 수 있는 성능을 갖고 있다고 한다.\n\n", velocity, distance);
	...
	if (solution == SHOW)
	{
	    ...
	    printf("관련 공식은 처음속도, 나중속도, 가속도, x변위 를 이용하여 구하는 공식입니다.\n");
		printf("먼저 처음속도, 나중속도, 가속도, x변위 를 구하세요.\n");
		printf("공식은 velocity_f^2 - velocity_i^2 = 2 * acceleration * x_displacement 입니다.\n\n");

		x_displacement = displacement_f - displacement_i;
		
		printf("acceleration 이외에 velocity_f, velocity_i, x_displacement를 구하면 다음과 같습니다.\n");
		printf("velocity_f %.0lf m/s,  velocity_i %.0lf m/s, x_displacement %.0lf m 입니다.\n", velocity_f, velocity_i, x_displacement);

		velocity = velocity_f - velocity_i;

		printf("위 식을 정리해주면 (%.0lf km/h)^2 = 2 * accelertaion * %.0lf m 입니다  \n\n", velocity, x_displacement); 

	    velocity_m_s = velocity * 1000 / 3600;

		printf("위 문제에서 가속도를 구해줘야 하기 때문에 km/h 를 m/s로 바꿔줍니다.\n");
		printf("km/h를 m/s로 바꿔주는 식은 가속도(acceleration) * 1000 / 3600로 해주면 됩니다.\n");
		printf("km/h를 m/s로 바꿔준 값은 %.0lf m 입니다.\n", velocity_m_s);
	...
	if (answer == SHOW)
	{
		...
		printf("따라서 (%.0lf)^2 m/s = 2 * acceleration * %.0lf m 입니다.\n\n" , velocity_m_s, x_displacement);
		printf("그러므로 acceleration을 제외한 나머지를 정리해주면 가속도는 %.2lf m/s입니다.\n\n", acceleration);
